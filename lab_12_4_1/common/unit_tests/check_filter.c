...
START_TEST(test_calculate_avr_empty_array)
{
    ...
    ck_assert_int_eq(rc, LIB_ERR_SIZE);
}
END_TEST
...
START_TEST(test_fill_above_avr_all_above)
{
    ...
    int rc = fill_above_avr(src, src + 4, avr, &pres, pres + 10);
...
// Тестирование функции fill_above_avr - памяти меньше чем нужно
START_TEST(test_fill_above_avr_mem)
{
    const int src[] = {6, 7, 8, 9};
    int result[10];
    int *pres = result;
    ...
    int rc = fill_above_avr(src, src + 4, avr, &pres, pres + 1);
...
Suite *fill_above_avr_suite(void)
{
    ...
    tcase_add_test(tc, test_fill_above_avr_mem);
...
START_TEST(test_copy_above_avr_elements_above_avr)
{
    ...
    int *pb_dst = malloc(sizeof(int) * 2);
    int *pe_dst = pb_dst + 2;
    int rc = copy_above_avr(pb_src, pe_src, avr, &pb_dst, &pe_dst);
...
START_TEST(test_key_one_element_above_average)
{
    int src[] = {1, 2, 3, 4, 10};
    int expected[] = {10};
    int *pb_dst = malloc(sizeof(int) * 1);
    int *pe_dst = pb_dst + 1;
...
START_TEST(test_key_many_elements_above_average)
{
    ...
    int expected[] = {8, 9, 10};
    int *pb_dst = malloc(sizeof(int) * 3);
    int *pe_dst = pb_dst + 3;
